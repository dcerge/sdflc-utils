...
describe('Transformers tests', () => {
  ...
  test('Trasformers: flattenHierarchy', () => {
    const src = [
      {
        id: '1',
        value: 'root',
    ...
    expect(flattenHierarchy(src, 'children')).toEqual(dst);
  });

  test('Trasformers: getLowestLevelItems', () => {
    const src = [
      {
        id: '1',
        value: 'root',
    ...
    expect(getLowestLevelItems(src, 'id', 'parentId', 'children', true)).toEqual(dst);
  });
});
